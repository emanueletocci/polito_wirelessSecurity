...
if isempty(strfind(sPlatform,'N'))...
        && ~contains(sPlatform,'O') && ~contains(sPlatform,'10')...
        && ~contains(sPlatform,'11')
...
%%%%  OLD csvread by Google. Superseeded by readmatrix. 
% Moreover, modern smartphones logs throe in non-numeric letters that are 
% not managed by csvread
...
S = readmatrix(rawCsvFile,'Range',[2 1],'TrailingDelimitersRule','keep');
...
for i=1:M
    ...
    elseif i == find(strcmp(header,'CodeType'))
        formatSpec = sprintf('%s %%c',formatSpec);
    elseif 0
        %ints
