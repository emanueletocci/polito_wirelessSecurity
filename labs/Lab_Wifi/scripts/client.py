...
def main():
    ...
    print(f"\nRunning 10 tests... Logs saved to {log_file}")
    ...
    if tputs:
        ...
        print(
            f"Avg: {statistics.mean(tputs):.2f} | Min: {min(tputs):.2f} | Max: {max(tputs):.2f} | StdDev: {(statistics.stdev(tputs) if len(tputs)>1 else 0):.2f}"
        )
